      ...
      <div class="example-box-wrapper">
         <div class="row">
            <div class="content-box pad10A center-margin col-md-12">
               <div id="map-cluster" style="height: 430px;"></div>
            </div>
         </div>
   ...
   <script>
      $(document).ready(function() {
         var map = new daum.maps.Map(document.getElementById('map-cluster'), { // 지도를 표시할 div
            center : new daum.maps.LatLng(37.5555048939659, 126.97375281464888), // 지도의 중심좌표 
            level : 9  // 지도의 확대 레벨 
         });      

         // 마커 이미지 바꾸기
//           var imageSrc = '../resources/image/marker-64.png';
//           var imageSize = new daum.maps.Size(64, 69); // 마커이미지의 크기입니다
//           var markerImage = new daum.maps.MarkerImage(imageSrc, imageSize);  // 마커이미지의 옵션입니다. 마커의 좌표와 일치시킬 이미지 안에서의 좌표를 설정합니다.
            ...
            $.ajax({
               ...
               success : function(resp) {
                  ...
                  $.each(resp,function(i,position) {
                     var c = position.name+ " : "+ position.road;
                     ...
                     var infowindow = new daum.maps.InfoWindow({
                        content : '<div style="width:200px;text-align:center;padding:6px 0;">'+markerTitle+'</div>',
                        removable : false
                     });

                     (
                           function(markers,infowindow) {// 마커에 mouseover 이벤트를 등록하고 마우스 오버 시 인포윈도우를 표시합니다 
                        
                              daum.maps.event.addListener(markers,'mouseover',function() {infowindow.open(map,markers);
                     ...
                     daum.maps.event.addListener(markers,'click',function() {// 현재 지도 레벨에서 1레벨 확대한 레벨          
                     var level = map.getLevel() - 1; // 지도를 클릭된 클러스터의 마커의 위치를 기준으로 확대합니다
                     map.setLevel(level,
                        {
                           anchor : map.setCenter(markerPosition)
                        });
                     });
                              ...
                              error : function(e) {
                                 alertify.alert('데이터로드실패,새로고침을 해주세요');
                              }
                           });
                  })
   </script>
